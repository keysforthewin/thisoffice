...
/**
 * Reframe the painting: applied locally at once (the wheel has to feel
 * immediate) and persisted on a trailing debounce, so one gesture is one write.
 */
export function reframeWallArt(patch: { zoom?: number; panX?: number }): void {
  ...
  saveTimer = setTimeout(() => {
    ...
    fetch('/api/decor/wallart', {
      ...
      body: JSON.stringify({ zoom: art.zoom, panX: art.panX }),
    }).catch(() => {});
  }, FRAMING_SAVE_MS);
}
